...
.defines(function() {
    ig.SplashLoader = ig.Loader.extend({
        ...
        splashDesktop: new ig.Image('media/graphics/sprites/bg/bg-load.png'),
        splashTitle: new ig.Image('media/graphics/splash/loading/title.png'),
        splashBgBar: new ig.Image('media/graphics/splash/loading/bg-load.png'),
        splashBarLoad: new ig.Image('media/graphics/splash/loading/bar-load.png'),
        ...
        addText:function(x, y, text, fontSize, fontType, settings){
            var showText = new EntityCustomText(x, y, settings);
        ...
        draw: function() {
            ...
            this.splashBgBar.draw((ig.system.width-this.splashBgBar.width)/2,450-this.splashBgBar.height/2);
            var scaleX=this._drawStatus;
            // console.log(this._drawStatus);
            ig.system.context.drawImage(this.splashBarLoad.data,0,0,this.splashBarLoad.width*scaleX,this.splashBarLoad.height,
                (ig.system.width-this.splashBgBar.width)/2+10,450-this.splashBarLoad.height/2,
                this.splashBarLoad.width*scaleX,this.splashBarLoad.height
            );
            //Animation run
			this.animate();
   ...
   //          );	
            ...
            this.splashTitle.draw((ig.system.width-this.splashTitle.width)/2,200-this.splashTitle.height/2);
            var fontSize=20;
            var posY=490;
            ...
            ctx.lineWidth=8;
            ctx.font = (fontSize) + "px" + " franks";
            ctx.strokeText(_STRINGS.Splash.Loading, (ig.system.width - ig.system.context.measureText(this.text).width)/2, posY);           
            ctx.restore();
            ...
            ctx.font = (fontSize) + "px" + " franks";
            ctx.fillText(_STRINGS.Splash.Loading, (ig.system.width - ig.system.context.measureText(this.text).width)/2, posY);           
            ctx.restore();

            // ctx.save();
            // ctx.fillStyle = this.fontColor;
            // ctx.font = fontSize + "px" + " " + this.fontStyle;
            // ctx.fillText(_STRINGS.Splash.Loading, this.pos.x - ig.system.context.measureText(this.text).width/2, this.pos.y);         
            // ctx.restore();
